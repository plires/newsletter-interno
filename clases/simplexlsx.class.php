	...
	private function _columnIndex( $cell = 'A1' ) {
		...
		if (preg_match("/([A-Z]+)(\d+)/", $cell, $matches)) {
			...
			for ($i = $colLen-1; $i >= 0; $i--)
				// $index += (ord($col{$i}) - 64) * pow(26, $colLen-$i-1); linea original.
				$index += (ord($col[$i]) - 64) * pow(26, $colLen-$i-1); //Se cammbiaron { } por [ ] cuando se actualizo a php 7.4 (PHP Deprecated)
