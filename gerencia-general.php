<?php 
  ...
  if (!$_SESSION || $_SESSION['sector_code'] != 'gerencia-general' && $_SESSION['sector_code'] != 'all') { 
    session_destroy();
    header('Location: ./');
  } else {
  ?>
                ...
                <div class="card-body">
                  <ul v-if="newslettersCurrentYear" class="pagination pagination-month justify-content-center">
                    ...
                    <li v-for="newsletter in newslettersCurrentYear" :key="newsletter.id" :class="['page-item', newsletterActive(newsletter.month)]">
                      <button class="page-link" @click="setCurrentNewsletter(newsletter.id, 'gerencia_general', 'preview_gerencia_general')">
