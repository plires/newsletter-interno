<?php 
  ...
  // Acceso permitido a sector Operaciones y Admin unicamente.
  if (!$_SESSION || $_SESSION['sector_code'] != 'operaciones' && $_SESSION['sector_code'] != 'all') { 
    session_destroy();
    header('Location: ./');
  } else {
  ?>
...
<head>
  ...
  <title>Vistage - Newsletter, Operaciones</title>
...
<body class="hold-transition sidebar-mini layout-fixed">
  <div id="app" class="wrapper operaciones">
      ...
      <section class="content">
        ...
        <div class="container">
                ...
                <div class="card-body">
                  <ul v-if="newslettersCurrentYear" class="pagination pagination-month justify-content-center">
                    ...
                    <li v-for="newsletter in newslettersCurrentYear" :key="newsletter.id" :class="['page-item', newsletterActive(newsletter.month)]">
                      <button class="page-link" @click="setCurrentNewsletter(newsletter.id, 'operaciones', 'preview_operaciones')">
          ...
          <section v-if="currentNewsletter.execute == 0" class="row">
            <div id="preview_operaciones" class="col-md-12">
              <div class="card card-primary">
                ...
                <div v-html="currentNewsletter.operaciones" class="card-body">
                </div>

                <div class="card-footer">
                  <button id="btnEditar" type="button" @click="initSummerNote('comment_operaciones', 'operaciones', 'preview_operaciones')" class="btn btn-primary float-right">Editar</button>
                </div>
                
              </div>
            </div>

            <div id="operaciones" class="col-md-12">
              ...
              <div class="card card-warning">
                ...
                <!-- form start -->
                <form method="post" @submit.prevent="submitForm(currentNewsletter, 'comment_operaciones', 'operaciones', 'preview_operaciones')">

                  <div class="card-body">
                    <div class="form-group">
                      <textarea id="comment_operaciones" name="comment_operaciones" class="form-control summernote" rows="3"></textarea>
                    </div>
                  </div>

                  <div class="card-footer">
                    <button id="btnGuardar" type="submit" class="btn btn-warning float-right ml-3">Guardar</button>
                    <button id="btnCancelar" type="button" @click="cancelEditNewsletter('operaciones', 'preview_operaciones')" class="btn btn-default float-right">Cancelar</button>
                  </div>
